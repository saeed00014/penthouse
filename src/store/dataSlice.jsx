  ...
  reducers: {
    getAllCards(state, action) {
      ...
      state.cards.map((card) => {
        (state.totalitems = state.totalitems + card.quantity / 2),
          (state.totalprice =
            state.totalprice +
            (card.quantity / 2) * card.price -
            ((card.quantity / 2) * card.price * card.discountPrecent) / 100);
      });
    },
    addcard(state, action) {
      const itemtoPush = action.payload;
      const itemtoPushPrice =
        itemtoPush.price -
        (itemtoPush.price * itemtoPush.discountPrecent) / 100;

      const foundedProductIndex = state.cards.findIndex(
        (product) => product.id == itemtoPush.id
      );

      const foundedProduct = state.cards[foundedProductIndex];

      const foundedProductPrice =
        foundedProduct &&
        foundedProduct.price -
          (foundedProduct.price * foundedProduct.discountPrecent) / 100;

      const foundedProductQuantity = foundedProduct && foundedProduct.quantity;

      foundedProductIndex >= 0
        ? foundedProductQuantity < 4
          ? ((state.cards[foundedProductIndex] = {
              ...foundedProduct,
              quantity: foundedProductQuantity + 1,
            }),
            (state.totalitems = itemtoPush.isAvailable
              ? state.totalitems + 1
              : state.totalitems),
            (state.totalprice = itemtoPush.isAvailable
    ...
    deletecard(state, action) {
      ...
      const foundedProduct = state.cards.find(
        (product) => product.id == itemtoDelete.id
      );
      const itemtoDeletePrice =
        (itemtoDelete.price -
          (itemtoDelete.price * itemtoDelete.discountPrecent) / 100) *
        itemtoDelete.quantity;

      const itemtoDeleteQuantity = foundedProduct.quantity;

      state.totalitems = itemtoDelete.isAvailable
        ? state.totalitems - itemtoDeleteQuantity
        : state.totalitems;

      state.totalprice = itemtoDelete.isAvailable
    ...
    selectquantitycard(state, action) {
      ...
      const foundedProduct = state.cards.find(
        (product) => product.id == patchedItem.id
      );
      ...
      const patchedItemPrice =
        (patchedItem.price -
          (patchedItem.price * patchedItem.discountPrecent) / 100) *
        patchedItem.quantity;
